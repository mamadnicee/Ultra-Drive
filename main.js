...
if(!('ontouchstart' in window)){
  ...
  document.addEventListener("mousemove", e=>{ gsap.to(cursor,{ x:e.clientX, y:e.clientY, duration:0.1, ease:"power1.out" }); });
}
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  card4.addEventListener("click", function(){
    ...
    setTimeout(()=>{ anchor.scrollIntoView({ behavior:"smooth" }); }, 150);
  });
    ...
    output+=`
      <div class="capsule">
        <div class="capsule-strip">
          ${images.map(img=>`<img src="images-NeonVaultStream/${img}" alt="">`).join("")}
          ${images.map(img=>`<img src="images-NeonVaultStream/${img}" alt="">`).join("")}
        </div>
      </div>`;
  }
